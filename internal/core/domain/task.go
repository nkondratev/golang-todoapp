...
func (t *Task) Validate() error {
	...
	if titleLength < 1 || titleLength > 100 {
		return fmt.Errorf("invalid `Title` len: %d: %w",
			titleLength, core_errors.ErrInvalidArgument)
	}
	...
	if t.Description != nil {
		...
		if descriptionLength < 1 || descriptionLength > 1000 {
			return fmt.Errorf("invalid `Description` len: %d: %w",
				descriptionLength, core_errors.ErrInvalidArgument)
		}
	}

	if t.Completed {
		if t.CompletedAt == nil {
			return fmt.Errorf("`CompletedAt` can't be `nil` if  `Completed`==`true`: %w",
